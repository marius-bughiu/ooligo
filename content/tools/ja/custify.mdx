...
Custify は、SMB から mid-market 帯の B2B SaaS 向けに作られた Customer Success プラットフォームです。プロダクトの利用イベント、サブスクリプション/請求データ、サポートのシグナルを 360 度のアカウントプロファイルに集約し、ユーザーが定義した数式から health score を計算し、その score をトリガーにライフサイクル playbook（オンボーディング、アダプション、更新、リスク）を発火させます。[Gainsight](/ja/tools/gainsight/) の中核となる CS ワークフローから、エンタープライズの重さ、価格、6 か月の導入を取り除いたものと考えてください。形としては [ChurnZero](/ja/tools/churnzero/) や [Vitally](/ja/tools/vitally/) に最も近く、Gainsight の展開を回す RevOps 機能を持たない CS チームを真正面から狙っています。
