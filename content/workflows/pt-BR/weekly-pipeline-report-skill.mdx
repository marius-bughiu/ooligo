...
3. **Configure o extrator de snapshot.** O skill consome dois arquivos de snapshot; não chama a API do HubSpot diretamente. Configure um pequeno extrator (cron job, flow n8n, GitHub Action agendado) que acessa `/crm/v3/objects/deals` com as colunas que o skill espera (`deal_id`, `deal_name`, `owner_id`, `stage`, `amount`, `close_date`, `last_modified`, mais o que suas regras de segmento referenciam) e escreve um arquivo CSV ou JSON por segunda-feira em um diretório conhecido. O extrator é a parte desta stack específica do HubSpot.
