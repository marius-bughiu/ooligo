...
n8n is the open-source workflow automation tool that won the [Zapier](/en/tools/zapier/)-vs-Make-vs-n8n race for AI-native ops teams. Self-host for free or run their cloud. Visual workflow builder with first-class AI agent nodes, code nodes (Python/JS), and a marketplace of 9,000+ community templates.
...
- **MCP support.** n8n can act as both an MCP client (calling external MCP servers) and an MCP server (exposing your workflows to Claude/[Cursor](/en/tools/cursor/) as tools). One node bridges your stack to any agent.
...
- **9,000+ templates.** The community library covers the most common patterns. Lead routing, enrichment loops, [Slack](/en/tools/slack/)-to-CRM, Stripe-to-data-warehouse — start from a template, customize.
