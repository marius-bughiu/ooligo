...
An [n8n](/en/tools/n8n/) flow that catches every inbound demo request the moment it lands, scores it against your ICP rubric with Claude, enriches it with firmographic data, and routes it to a self-serve flow, an SDR queue by territory, or a [Slack](/en/tools/slack/) page to the AE on call. The bundle at `apps/web/public/artifacts/inbound-lead-triage-n8n/` ships the complete export plus a `_README.md` covering import, credential setup, and per-branch verification.
