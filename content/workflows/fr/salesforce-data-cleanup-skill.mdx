...
Un [Claude](/fr/tools/claude/) Skill qui scanne Salesforce pour les déchets de données qui distordent silencieusement votre reporting — comptes dupliqués, contacts orphelins, leads indésirables, téléphones malformés, inadéquations compte-contact et valeurs d'étape qui violent la définition du funnel — puis propose des corrections sous forme de CSV que l'opérateur approuve avant qu'un quelconque write n'atterrisse. Le Skill n'écrit jamais sans un dry-run explicite plus l'approbation humaine, et chaque changement appliqué est consigné dans un objet d'audit personnalisé afin de pouvoir être annulé.
