...
1. **Systèmes.** Choisir, configurer et maintenir le stack d'outils GTM : CRM (HubSpot, Salesforce), séquenceur (Outreach, Salesloft, Apollo), enrichissement de données (Clay, ZoomInfo), conversation intelligence (Gong), automatisation (n8n, [Zapier](/fr/tools/zapier/)), orchestration IA (Claude + MCP).
...
Une équipe RevOps moderne écrit des Claude Skills, construit des agents n8n qui trient les leads inbound, déploie des [Cursor](/fr/tools/cursor/) rules pour les profils ingénierie des données, et traite le stack d'outils IA comme un domaine de première classe — pas une expérimentation annexe.
