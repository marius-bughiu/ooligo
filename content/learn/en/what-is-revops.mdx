...
1. **Systems.** Choosing, configuring, and maintaining the GTM tool stack: CRM (HubSpot, Salesforce), sequencer (Outreach, Salesloft, Apollo), data enrichment (Clay, ZoomInfo), conversation intelligence (Gong), automation (n8n, [Zapier](/en/tools/zapier/)), AI orchestration (Claude + MCP).
...
A modern RevOps team writes Claude Skills, builds n8n agents that triage inbound, deploys [Cursor](/en/tools/cursor/) rules for the data-engineer types, and treats the AI tool stack as a first-class domain — not a side experiment.
