...
Vous avez au moins 30 no-shows de démo par mois dans l'équipe. En dessous de ce volume, le commercial peut les récupérer à la main et vous n'avez pas besoin d'automatisation ; le mode d'échec de l'automatisation (un email légèrement décalé au mauvais moment) devient pire que le mode d'échec de la récupération manuelle (l'oubli). Vous êtes déjà sur [HubSpot](/fr/tools/hubspot/) pour les réunions (ou Chili Piper écrit en retour dans HubSpot) et les boîtes mail des commerciaux sont sur Google Workspace. Vous avez un signal de no-show honnête — ce qui signifie que HubSpot marque réellement les réunions comme `no_show` plutôt que de les laisser vides — et vos commerciaux ont des liens de planification fonctionnels.
