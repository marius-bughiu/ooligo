...
**Model flexibility.** Protégé offers access to multiple frontier models — including GPT-5, Claude Sonnet 4, GPT-4o, and OpenAI o3 — within the LexisNexis environment, per the platform launch announcement. This gives power users the ability to select the best model for a given task. CoCounsel is built on the Anthropic Claude Agent SDK (as disclosed by Thomson Reuters in 2025) and does not offer user-level model switching.
