...
10. **Greenhouse — List Stale Unscheduled Interviews** — webhook が見逃されたか配信が失敗した面接を見つけるために、Greenhouse Harvest `GET /v1/scheduled_interviews?status=to_be_scheduled&created_before=<48h-ago>` を呼び出します。
11. **Split Into Items** — レスポンス配列を応募ごとの処理のために個別アイテムに分割します。
...
*Guard:* 毎朝 08:00 ET の日次バックアップ cron が、48 時間以上前に作成された `to_be_scheduled` 状態の面接を Greenhouse で検索して再処理します。48 時間の閾値は、まだ送信中の webhook がある可能性のある最近作成された面接を再処理しないためのものです。
...
- `apps/web/public/artifacts/interview-scheduling-resolver-n8n/interview-scheduling-resolver-n8n.json` — n8n フローのエクスポート（12 ノード、完全に設定済み、プレースホルダー credential 名付き）
