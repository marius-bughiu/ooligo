...
- **C'est une plomberie réellement native de l'IA.** Un serveur MCP hébergé par Pylon (mcp.usepylon.com, OAuth) permet à Claude ou [Cursor](/fr/tools/cursor/) d'interroger et de mettre à jour les issues et les comptes, en plus d'une REST API documentée et de webhooks pour la long tail.

## Pricing

Pylon a retiré sa tarification publique. `usepylon.com/pricing` redirige désormais en 301 vers la réservation de démo, si bien que chaque chiffre ci-dessous correspond au **dernier tarif publié** et non à un montant vérifiable aujourd'hui — servez-vous-en comme d'un point d'ancrage de négociation, pas d'un devis. Dernier tarif publié, par siège et facturé annuellement : Starter 59 $/siège/mois, Professional 89 $/siège/mois, Enterprise 139 $/siège/mois. Starter et Professional imposaient un minimum de 3 sièges ; Enterprise en exigeait 7 (soit un plancher d'environ 973 $/mois). Il n'y avait pas de tier free, seulement un trial. C'est avec l'IA que la facture grimpait : les AI Assistants ajoutaient 50 $/siège/mois, les AI Agents démarraient à 100 $/mois et évoluaient avec le volume d'issues sur 30 jours, et l'Account Intelligence coûtait 10 $ par compte client/mois (minimum 50 comptes). Une petite équipe qui veut le stack IA complet doit prévoir un budget bien au-dessus du prix de siège affiché — de façon réaliste 130 à 200 $ par siège tout compris une fois Assistants et Agents activés.
...
- **La tarification passe désormais par une démo, et les add-ons d'IA sont là où atterrit le coût réel.** Les 59 $ autrefois affichés correspondaient à un siège de la plateforme de base ; les fonctionnalités de déflexion et de signaux qui justifient le choix de Pylon étaient toutes facturées en sus. La garde : faites chiffrer Assistants + Agents + Account Intelligence comme des lignes distinctes par rapport à votre nombre de sièges et votre volume d'issues dès le premier échange, et faites confirmer le tarif par siège directement plutôt que de vous fier à une liste tierce qui a recopié l'ancienne page publique.
