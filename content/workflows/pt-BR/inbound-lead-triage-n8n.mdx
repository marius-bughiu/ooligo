...
Um flow [n8n](/pt-BR/tools/n8n/) que captura cada requisição de demo inbound no momento em que ela chega, pontua contra o seu rubric de ICP com Claude, enriquece com dados firmográficos e roteia para um flow de self-serve, uma fila de SDR por território ou uma página [Slack](/pt-BR/tools/slack/) para o AE de plantão. O bundle em `apps/web/public/artifacts/inbound-lead-triage-n8n/` inclui o export completo mais um `_README.md` cobrindo importação, configuração de credenciais e verificação por ramificação.
