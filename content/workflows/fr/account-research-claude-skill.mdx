...
3. **Configurez le format de sortie.** La sortie par défaut est un brief Markdown avec cinq sections. Modifiez `SKILL.md` si votre équipe préfère des blocs [Notion](/fr/tools/notion/), un Google Doc ou un résumé adapté à Slack.
...
- **Données publiques périmées.** Le scraping LinkedIn se casse fréquemment et les transcriptions d'appels de résultats ont un trimestre de retard. Si la fraîcheur compte, intégrez en amont une source payante comme [ZoomInfo](/fr/tools/zoominfo/) ou Apollo.
