...
A [Claude](/en/tools/claude/) Skill that ingests an AE's last ten [Gong](/en/tools/gong/) calls and outputs a coaching note in a fixed three-things-working / two-things-to-tighten / one-specific-exercise shape. Built for managers who want consistent weekly coaching but do not have ten hours a week to listen back to calls. The skill writes the draft; the manager edits, contextualizes with off-call signal, and delivers in the next one-on-one. Auto-send is explicitly out of scope.
