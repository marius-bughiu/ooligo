...
**vs le scoring d'expansion / product-qualified-lead propre à [Gainsight](/fr/tools/gainsight/) ou Pendo.** Si vous payez déjà les scorecards d'expansion de Gainsight ou les signaux PQL de Pendo, ils couvrent bien les dimensions usage et sièges et n'exigent aucun build. Ce qu'ils ne font pas, c'est lire le langage des appels Gong — le détenteur de budget qui interroge sur un module qu'il ne possède pas encore — et le replier dans le même classement. Cette Skill existe pour ajouter le signal de conversation et émettre un play, pas seulement un score. Si vous avez Gainsight, faites tourner ceci comme la couche d'intention Gong et laissez Gainsight posséder le rollup d'usage ; ils sont complémentaires.
