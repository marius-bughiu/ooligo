...
- **Integration ecosystem.** Every serious B2B tool — Outreach, Salesloft, [Gong](/en/tools/gong/), Clay, ZoomInfo, Marketo, Pardot, etc. — has a deep, native Salesforce integration before any other CRM.
- **Agentforce + MCP.** Salesforce launched Agentforce as the AI agent layer in 2024; the official MCP server now also lets you wire [Claude](/en/tools/claude/)/Cursor directly into your Salesforce data. The AI quality is improving but most ops teams still pair it with Claude/Gong rather than relying on Agentforce alone.
...
- UX is dated relative to [HubSpot](/en/tools/hubspot/) and the new wave (Attio, Folk). Reps complain.
