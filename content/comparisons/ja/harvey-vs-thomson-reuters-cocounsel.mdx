...
[Harvey](/ja/tools/harvey/) と [Thomson Reuters CoCounsel](/ja/tools/thomson-reuters-cocounsel/) は、2026年第1四半期時点でAmLaw 100とMagic Circle法律事務所に最も広く展開されている2つのAIプラットフォームです。両者は四半期ごとに数百万件の法律文書を処理しており、Thomson Reutersの公開報告によると、2026年Q1だけで両社のAIエージェントが合計1,000万件以上の文書を処理しました。ただし、両者の構造的なアプローチは根本的に異なります。Harveyは、各事務所独自の文書でファインチューニングを行い、カスタムワークフローを構築する水平型AIプラットフォームです。CoCounselは、Thomson Reutersの独自コンテンツに最初から根付いた回答を提供するWestlawおよびPractical Law専用エージェントです。選択は、既存のリサーチスタックがWestlaw上で動いているかどうかによって大部分が決まります。
...
**文書タスクのベンチマーク性能。** 2025年2月に公開された独立系ベンチマークであるVals Legal AI Report（HarveyとCoCounselの両者が参加）では、Harveyは評価された6つのタスクのうち5つで最高スコアを獲得しました。Document Q&Aでは94.8%の精度（CoCounselの89.6%を上回る）、文書抽出では75.1%を達成しています。Harveyは4つのタスクで弁護士ベースラインの70.1%を超えました。これらの数値は両社が公開で引用したものであり、方向性の指標として扱ってください。
...
**採用率。** Harveyが報告する月間平均利用率92%と、AmLaw 100事務所の80社がHarveyを何らかの形で利用しているという事実（2025年12月、HarveyのシリーズC発表時点）は、展開後も利用が維持されることを示しており、エンタープライズ耐久性の信頼できるシグナルです。
