...
A Claude Skill that takes a batch of documents — typically a folder of email and attachments exported from an [eDiscovery](/en/learn/ediscovery/) review platform, or a directory of contracts pulled from CLM — and runs a first-pass [privilege review](/en/learn/privilege-review/). For each document it emits one of `privileged`, `not-privileged`, or `borderline-needs-attorney`, backed by citation-grounded evidence spans, plus a draft [privilege log](/en/learn/privilege-log-format/) entry for every document classified `privileged`.
...
- [Claude](/en/tools/claude/) — Tier-A endpoint only (Anthropic API enterprise tier or your enterprise Claude tenant)
- [Relativity](/en/tools/relativity/), [Everlaw](/en/tools/everlaw/), or [DISCO](/en/tools/disco/) — source of the document export and metadata CSV
- The skill bundle at `apps/web/public/artifacts/privilege-review-batch-skill/`
- Related reading: [eDiscovery](/en/learn/ediscovery/), [privilege review](/en/learn/privilege-review/), [privilege log format](/en/learn/privilege-log-format/), [EDRM model](/en/learn/edrm-model/), [AI policy for legal teams](/en/learn/ai-policy-for-legal-teams/)
