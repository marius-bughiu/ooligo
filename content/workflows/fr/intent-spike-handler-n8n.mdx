...
`Dedup Check — Fetch Static Data` lit le store de données statiques du workflow n8n pour une clé composée du domaine + la date du jour (`acme.com::2026-05-23`). Un 404 signifie premier spike de la journée — continuer. Un 200 signifie que le flow a déjà traité ce compte aujourd'hui — le nœud `Dedup Gate` retourne un tableau vide et l'exécution s'arrête silencieusement. La fenêtre journalière est le bon horizon de déduplication parce que les plateformes d'intention réévaluent souvent les comptes toutes les quelques heures&nbsp;; sans fenêtre, le même spike génèrerait 4–6 notifications par jour et par compte.

`Salesforce — Account Lookup` interroge Salesforce pour un Account dont le champ Website contient le domaine du spike. `Assignment Logic` utilise le résultat&nbsp;: si un Account a été trouvé, le propriétaire existant reçoit le spike. Si aucun Account n'a été trouvé, le nœud sélectionne un pool territorial en utilisant le pays de facturation du compte contre trois ensembles (AMER, EMEA, ROW). Les emails de pool et les handles Slack proviennent de variables d'environnement.
...
- **Les données statiques de déduplication s'accumulent indéfiniment.** Le nœud `Dedup Mark — Write Static Data` estampille une clé par domaine par jour, mais les données statiques du workflow n8n ne sont pas automatiquement purgées. Guard&nbsp;: planifiez une exécution de maintenance mensuelle qui appelle l'API n8n pour effacer les données statiques de ce workflow, ou ajoutez un cron secondaire qui supprime les clés de plus de 7 jours.
