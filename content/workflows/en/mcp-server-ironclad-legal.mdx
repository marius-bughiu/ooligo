...
Finally, skip this if you do not yet have an [AI policy for legal teams](/en/learn/ai-policy-for-legal-teams/) that covers Claude access to contract data. Stand the policy up first; then this server.
...
Self-hosted Python MCP server (the scaffold uses the official `mcp` SDK, `httpx`, `pydantic`) speaking to the Ironclad public API on the backend; [Claude](/en/tools/claude/) Desktop or Code on the front end. Optional: structured logging via `python-json-logger` piped to your matter-management audit trail; Sentry or OpenTelemetry export, with query strings and document bodies scrubbed before transmission.
