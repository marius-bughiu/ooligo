...
A Claude Skill that mines win/loss conversations in [Gong](/en/tools/gong/), reconciles them against [Salesforce](/en/tools/salesforce/) deal outcomes, and produces an internal battlecard a rep can read on the way into the call. The card is structured: positioning summary, where you win, where you lose, talk-tracks per objection (each marked internal or customer-facing), and a short "traps to avoid" list. It replaces the PMM-written battlecard that was true the day it shipped and stale by the next quarter.
