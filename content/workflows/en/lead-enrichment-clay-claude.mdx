...
A [Claude](/en/tools/claude/) Skill (`lead-enrichment`) that takes a [Clay](/en/tools/clay/) table row keyed on `domain` and returns three derived fields in one structured pass: a 2-sentence company summary, an ICP fit score 1-10 with a one-line reason, and a sub-50-word cold-email opener that cites a specific signal from the company's public footprint. The opener is always a draft for rep review — the skill refuses to be wired into an auto-send step.
