...
Exécutez ceci sur la cadence de votre réunion de forecast. Le pattern canonique est un batch dimanche soir indexé sur `week_ending`, avec le rapport tombant dans un canal [Slack](/fr/tools/slack/) avant le point du lundi matin des managers. Le mode opp unique est aussi valide — un réviseur du deal-desk peut exécuter le Skill contre un `Opportunity.Id` avant une réunion d'approbation tarifaire, ou un manager peut l'exécuter contre un seul deal avant un 1:1 pour ancrer la conversation dans les gaps spécifiques plutôt que dans un vague sentiment de « ça semble bloqué ».
